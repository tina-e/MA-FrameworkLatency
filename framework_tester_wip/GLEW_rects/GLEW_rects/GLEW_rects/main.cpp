...
void drawRects()
{
    for (int i = 0; i < 999; i++)
    {
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_x[i], random_x[i]);
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_x[i], random_y[i]);
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_y[i], random_x[i]);
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_y[i], random_x[i]);
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_x[i], random_y[i]);
        glColor3f(random_r[i], random_g[i], random_r[i]);
        glVertex2f(random_y[i], random_y[i]);
    }
    glColor3f(1, 1, 1);
    glVertex2f(-1, 1);
    glColor3f(1, 1, 1);
...
int main()
{
    ...
    {
        glClear(GL_COLOR_BUFFER_BIT); // maybe das hier? vllt macht das schon hell?
        ...
        glfwSwapBuffers(window); // vllt das hier?
        glfwPollEvents();
    }
