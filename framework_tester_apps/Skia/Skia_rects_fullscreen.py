...
def draw_rects(canvas):
    for i in range(NUM_RECTS):
        w = randint(100, 500)
        h = randint(100, 500)
        x = randint(100, WIDTH - w)
        y = randint(0, HEIGHT - h)
        r = randint(0, 255)
        g = randint(0, 255)
        b = randint(0, 255)
        canvas.drawRect(skia.Rect(x, y, w, h), skia.Paint(Color=skia.Color(r, g, b, 255)))
    canvas.drawRect(skia.Rect(0, 0, 100, 100), skia.Paint(Color=skia.ColorWHITE))
