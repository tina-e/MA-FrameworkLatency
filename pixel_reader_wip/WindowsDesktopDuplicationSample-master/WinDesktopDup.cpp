...
bool WinDesktopDup::CaptureNext() {
	...
	// Calculate the byte offset for the pixel of interest
	UINT rowPitch   = sr.RowPitch;                         // The number of bytes between rows of adjacent pixels
	UINT byteOffset = (1 * rowPitch) + (1 * sizeof(UINT)); // Assuming UINT is the color format

	// Now, you can access the color value at the specified pixel
	UINT colorValue = *reinterpret_cast<UINT*>(pixels + byteOffset);

	// Extract individual color components (assuming RGBA format)
